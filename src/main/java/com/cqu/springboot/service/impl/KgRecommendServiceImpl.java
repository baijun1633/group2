...
public class KgRecommendServiceImpl implements KgRecommendService {
    ...
    public List<RecommendItem> recommendByKg(Long userId, int limit) {
        ...
        if (!preferredCategoryIds.isEmpty()) {
            ...
            Collection<Map<String, Object>> records = neo4jClient.query(cypher)
                .bindAll(Map.of("categoryIds", preferredCategoryIds, "limit", limit))
                .fetch().all();
        ...
        if (!preferredAuthors.isEmpty()) {
            String cypher = "MATCH (b:Book)-[:WRITTEN_BY]->(a:Author) " +
                ...
                "LIMIT $limit";
            Collection<Map<String, Object>> records = neo4jClient.query(cypher)
                .bindAll(Map.of("authors", preferredAuthors, "limit", limit))
                .fetch().all();
            for (Map<String, Object> r : records) {
                results.add(buildItem(r, "KG",
        ...
        if (!preferredTags.isEmpty()) {
            String cypher = "MATCH (b:Book)-[:TAGGED_AS]->(t:Tag) " +
                ...
                "LIMIT $limit";
            Collection<Map<String, Object>> records = neo4jClient.query(cypher)
                .bindAll(Map.of("tags", preferredTags, "limit", limit))
                .fetch().all();
            for (Map<String, Object> r : records) {
                results.add(buildItem(r, "KG",
    ...
    @Override
    public List<RecommendItem> getSimilarBooks(Long bookId, int limit) {
        ...
        // 1跳：同作者
        String cypherAuthor = "MATCH (b1:Book {bookId: $bookId})-[:WRITTEN_BY]->(a:Author)<-[:WRITTEN_BY]-(b2:Book) " +
        ...
        Collection<Map<String, Object>> authorBooks = neo4jClient.query(cypherAuthor)
            .bindAll(Map.of("bookId", bookId, "limit", perType))
            .fetch().all();
        ...
        // 1跳：同分类
        String cypherCategory = "MATCH (b1:Book {bookId: $bookId})-[:BELONGS_TO]->(c:Category)<-[:BELONGS_TO]-(b2:Book) " +
            "WHERE b1.bookId <> b2.bookId " +
        ...
        // 1跳：同标签
        String cypherTag = "MATCH (b1:Book {bookId: $bookId})-[:TAGGED_AS]->(t:Tag)<-[:TAGGED_AS]-(b2:Book) " +
            "WHERE b1.bookId <> b2.bookId " +
    ...
    @Override
    public List<RecommendItem> getExtendedBooks(Long bookId, int limit) {
        Books sourceBook = booksMapper.selectById(bookId);
        if (sourceBook == null) return Collections.emptyList();
        ...
        // 3跳路径1：Book → Category → Book → Author → Book
        String cypher1 = "MATCH (b1:Book {bookId: $bookId})-[:BELONGS_TO]->(c:Category)<-[:BELONGS_TO]-(b2:Book)-[:WRITTEN_BY]->(a:Author)<-[:WRITTEN_BY]-(b3:Book) " +
        ...
        Collection<Map<String, Object>> path1 = neo4jClient.query(cypher1)
            .bindAll(Map.of("bookId", bookId, "limit", perPath))
            .fetch().all();
        for (Map<String, Object> r : path1) {
            results.add(new RecommendItem(
        ...
        // 3跳路径2：Book → Author → Book → Tag → Book
        String cypher2 = "MATCH (b1:Book {bookId: $bookId})-[:WRITTEN_BY]->(a:Author)<-[:WRITTEN_BY]-(b2:Book)-[:TAGGED_AS]->(t:Tag)<-[:TAGGED_AS]-(b3:Book) " +
            "WHERE b1.bookId <> b3.bookId AND b2.bookId <> b3.bookId " +
            "RETURN DISTINCT b3.bookId AS bookId, b3.title AS title, b3.avgRating AS avgRating, " +
        ...
        // 3跳路径3：Book → Tag → Book → Category → Book
        String cypher3 = "MATCH (b1:Book {bookId: $bookId})-[:TAGGED_AS]->(t:Tag)<-[:TAGGED_AS]-(b2:Book)-[:BELONGS_TO]->(c:Category)<-[:BELONGS_TO]-(b3:Book) " +
            "WHERE b1.bookId <> b3.bookId AND b2.bookId <> b3.bookId " +
            "RETURN DISTINCT b3.bookId AS bookId, b3.title AS title, b3.avgRating AS avgRating, " +
